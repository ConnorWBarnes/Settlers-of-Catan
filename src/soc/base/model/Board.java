...
public class Board {
	...
	private HashMap<Integer, LinkedList<Tile>> numberTokenMap;//Key is a number token, value is an array of the tiles that have that number token
	...
	public Board() {
		...
		//Create the tiles and add them to the board randomly
		ArrayList<Tile> defaultTileList = generateDefaultTiles();
		int randomNum;
		for (i = 0; i < tileMap.length; i++) {
			randomNum = (int) (Math.random() * defaultTileList.size());
			tileMap[i] = defaultTileList.remove(randomNum);
		}
		//Add the number tokens to the board
		setNumberTokens();
		//Create the cornerMap
		for (i = 0; i < cornerMap.length; i++) {
			cornerMap[i] = new Corner();
		}
		//Give certain harbor values for certain corners determined by the game board
		setHarbors();
		//Populate adjacentCornerLocs list for each corner
		populateAdjacentCornerLocsForCorners();
		populateAdjacentTileLocs();
		//Create the roadMap
		for (i = 0; i < roadMap.length; i++) {
			roadMap[i] = new Road();
		}
		//Populate the adjacentRoadLocs and adjacentCornerLocs lists for each road
		populateAdjacentRoadLocsForRoads();
		populateAdjacentCornerLocsForRoads();
		populateAdjacentRoadLocsForCorners();
	}
    ...
    /**
     * Adds a road of the specified color to the board at the specified location.
    ...
    //Sets the adjacent road locations for each road
	private void populateAdjacentRoadLocsForRoads()	{
    ...
    //Sets the adjacent corner locations for each road
	private void populateAdjacentCornerLocsForRoads() {
		int i;
		for (i = 0; i < 6; i++)	{
    ...
    //Sets the adjacent tile locations for each corner
	private void populateAdjacentTileLocs()	{
    ...
    //Sets the adjacent road locations for each corner
	private void populateAdjacentRoadLocsForCorners() {
